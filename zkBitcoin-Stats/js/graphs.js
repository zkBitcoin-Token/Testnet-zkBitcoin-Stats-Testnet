...
var ethblockstart = 14339912
var adjustAverageRewardTimeGraph = 27.5
...
function generateHashrateAndBlocktimeGraph(eth, target_cv_obj, era_cv_obj, tokens_minted_cv_obj) {
  ...
  function getHashrateDataFromDifficultyAndErasPerBlockData(difficulty_data, eras_per_block_data) {
    var expected_eras_per_block = 1/270; //76.5/* should be 40 times slower than ethereum (with 15-second eth blocks) */
  ...
  var expected_eras_per_block = 1/270; /* should be 40 times slower than ethereum (with 15-second eth blocks) */
    var difficulty_data_index = 0;
    var difficulty_change_block_num = 0;
    var chart_data = []
    for (var step = 0; step < eras_per_block_data.length; step++) {
...
function updateGraphData(history_days, num_search_points) {
  ...
  log("_SECONDS_PER_ETH_BLOCK..."+eth_blocks_per_day);
    ...
    let max_blocks = history_days*eth_blocks_per_day/100;
